...
async function pkpassImageHandler(imageBuffer, imageHost) {
  ...
  if (config.googleStorageBucket) {
    ...
    console.warn(
      `Cannot determine public host for image ${imageName} as googleStorageBucket config is not defined. The resulting pass will not be savable to Google Wallet without adding the URL for this image.`,
    );
    imageHost = '';
  }
